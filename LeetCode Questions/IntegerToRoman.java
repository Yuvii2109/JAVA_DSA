...
public class IntegerToRoman {
    public String intToRoman(int num) {
        int[] correspondingNum = {1000, 900, 500, 400, 100, 90, 50, 40, 10, 9, 5, 4, 1};
        String[] correspondingRoman = {"M", "CM", "D", "CD", "C", "XC", "L", "XL", "X", "IX", "V", "IV", "I"};
        // Build the result by repeatedly subtracting the largest possible value and appending its symbol.
        StringBuilder roman = new StringBuilder();
        for(int i = 0; i < correspondingNum.length; i++){
            while(num >= correspondingNum[i]){
                num -= correspondingNum[i];
                roman.append(correspondingRoman[i]);
            }
        }
